...
<div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
	...
	<button id="open-address-modal"
			class="flex items-center gap-2 bg-primary text-white px-6 py-3 rounded-xl font-bold hover:bg-primary/90 transition-all shadow-md acct-btn-add-address cursor-pointer">
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
	...
	<div class="glass-card rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col">
		...
		<div class="px-6 py-4 bg-slate-50/50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex gap-3">
			<button class="edit-address-btn flex-1 py-2 rounded-lg text-xs font-bold border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-700 transition-colors flex items-center justify-center gap-1 acct-btn-edit cursor-pointer"
					data-label="<?php echo esc_attr( $has_shipping ? __( 'Home Estate', 'luxe-landscape' ) : 'Home Estate' ); ?>"
					data-name="<?php echo esc_attr( $has_shipping ? $shipping_address['first_name'] . ' ' . $shipping_address['last_name'] : 'Julianne Sterling' ); ?>"
					data-street="<?php echo esc_attr( $has_shipping ? $shipping_address['address_1'] : '842 Verdant Valley Road' ); ?>"
					data-city="<?php echo esc_attr( $has_shipping ? $shipping_address['city'] : 'Pacific Palisades' ); ?>"
					data-postal="<?php echo esc_attr( $has_shipping ? $shipping_address['postcode'] : '90272' ); ?>"
					data-apt="<?php echo esc_attr( $has_shipping ? $shipping_address['address_2'] : '' ); ?>">
				<span class="material-symbols-outlined text-sm">edit</span> <?php esc_html_e( 'Edit', 'luxe-landscape' ); ?>
			</button>
			<button class="flex-1 py-2 rounded-lg text-xs font-bold border border-slate-200 dark:border-slate-700 text-slate-400 hover:text-red-500 hover:border-red-200 transition-colors flex items-center justify-center gap-1 acct-btn-delete cursor-pointer">
	...
	<div class="glass-card rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col">
		...
		<div class="px-6 py-4 bg-slate-50/50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex gap-3">
			<button class="edit-address-btn flex-1 py-2 rounded-lg text-xs font-bold border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-700 transition-colors flex items-center justify-center gap-1 acct-btn-edit cursor-pointer"
					data-label="<?php echo esc_attr( $has_billing ? __( 'Studio Office', 'luxe-landscape' ) : 'Studio Office' ); ?>"
					data-name="<?php echo esc_attr( $has_billing ? $billing_address['first_name'] . ' ' . $billing_address['last_name'] : 'Sterling Designs LLC' ); ?>"
					data-street="<?php echo esc_attr( $has_billing ? $billing_address['address_1'] : '1200 Emerald Tower, Ste 402' ); ?>"
					data-city="<?php echo esc_attr( $has_billing ? $billing_address['city'] : 'Los Angeles' ); ?>"
					data-postal="<?php echo esc_attr( $has_billing ? $billing_address['postcode'] : '90015' ); ?>"
					data-apt="<?php echo esc_attr( $has_billing ? $billing_address['address_2'] : '' ); ?>">
				<span class="material-symbols-outlined text-sm">edit</span> <?php esc_html_e( 'Edit', 'luxe-landscape' ); ?>
			</button>
			<button class="flex-1 py-2 rounded-lg text-xs font-bold border border-slate-200 dark:border-slate-700 text-slate-400 hover:text-red-500 hover:border-red-200 transition-colors flex items-center justify-center gap-1 acct-btn-delete cursor-pointer">
				<span class="material-symbols-outlined text-sm">delete</span> <?php esc_html_e( 'Delete', 'luxe-landscape' ); ?>
...
<!-- ========================================
     ...
     ======================================== -->
<div id="address-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none opacity-0 transition-all duration-300 ease-out" style="backdrop-filter: blur(0px); background-color: rgba(0,0,0,0);">
	<div id="address-modal-panel" class="bg-background-light dark:bg-background-dark w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden border border-white/20 dark:border-slate-700 max-h-[90vh] overflow-y-auto transform scale-95 translate-y-4 opacity-0 transition-all duration-300 ease-out">
		<!-- Modal Header -->
		<div class="px-8 pt-8 pb-4 flex justify-between items-center">
			<div>
				<h2 id="modal-title" class="font-display text-3xl font-bold text-primary dark:text-primary acct-modal-title"><?php esc_html_e( 'Add New Address', 'luxe-landscape' ); ?></h2>
				<p id="modal-desc" class="text-slate-500 dark:text-slate-400 text-sm mt-1 acct-modal-desc"><?php esc_html_e( 'Enter your details for biophilic delivery.', 'luxe-landscape' ); ?></p>
			</div>
			<button id="close-address-modal" class="text-slate-400 hover:text-slate-600 dark:hover:text-white transition-colors">
				<span class="material-symbols-outlined text-3xl cursor-pointer">close</span>
			</button>
		</div>
		...
		<form class="px-8 py-4 space-y-6">
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div class="flex flex-col gap-1.5">
					...
					<input id="addr-label" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-emerald-800/50 bg-white dark:bg-emerald-900/20 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all dark:text-white"
				...
				<div class="flex flex-col gap-1.5">
					...
					<input id="addr-name" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-emerald-800/50 bg-white dark:bg-emerald-900/20 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all dark:text-white"
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div class="flex flex-col gap-1.5">
					...
					<input id="addr-street" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-emerald-800/50 bg-white dark:bg-emerald-900/20 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all dark:text-white"
				...
				<div class="flex flex-col gap-1.5">
					...
					<input id="addr-city" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-emerald-800/50 bg-white dark:bg-emerald-900/20 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all dark:text-white"
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div class="flex flex-col gap-1.5">
					...
					<input id="addr-postal" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-emerald-800/50 bg-white dark:bg-emerald-900/20 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all dark:text-white"
				...
				<div class="flex flex-col gap-1.5">
					...
					<input id="addr-apt" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-emerald-800/50 bg-white dark:bg-emerald-900/20 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all dark:text-white"
		...
		<div class="px-8 py-8 flex flex-col sm:flex-row-reverse gap-4 bg-slate-50 dark:bg-slate-800/50 mt-4">
			<button id="modal-save-btn" class="w-full sm:w-auto px-8 py-4 bg-primary hover:bg-primary/90 text-white font-bold rounded-xl shadow-lg shadow-primary/20 transition-all flex items-center justify-center gap-2 acct-modal-save cursor-pointer">
				<span class="material-symbols-outlined">save</span>
				<span id="modal-save-text"><?php esc_html_e( 'Save Address', 'luxe-landscape' ); ?></span>
			</button>
			<button id="cancel-address-modal" class="w-full sm:w-auto px-8 py-4 text-slate-500 dark:text-slate-400 font-medium hover:text-primary transition-colors acct-modal-cancel cursor-pointer">
