	...
	<form class="woocommerce-EditAccountForm edit-account" action="" method="post" <?php do_action( 'woocommerce_edit_account_form_tag' ); ?>>
		...
		<div class="flex-1 w-full space-y-6">
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
				...
				<!-- Phone Number -->
				<div class="flex flex-col gap-2">
					<label class="text-sm font-bold text-slate-700 dark:text-slate-300 ml-1 acct-label-phone" for="account_phone"><?php esc_html_e( 'Phone Number', 'luxe-landscape' ); ?></label>
					<div class="relative">
						<span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 font-medium">+966</span>
						<input class="w-full bg-background-light dark:bg-emerald-900/20 border border-slate-200 dark:border-emerald-800/50 rounded-xl pl-14 pr-4 py-3.5 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all outline-none text-slate-800 dark:text-white font-medium"
							   type="tel" name="account_phone" id="account_phone"
							   value="<?php echo esc_attr( get_user_meta( $user->ID, 'billing_phone', true ) ); ?>"
							   placeholder="<?php esc_attr_e( '555-0123', 'luxe-landscape' ); ?>" />
						<?php if ( get_user_meta( $user->ID, 'billing_phone', true ) ) : ?>
						<div class="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-1 text-green-600">
							<span class="material-symbols-outlined text-sm">verified</span>
							<span class="text-[10px] font-bold uppercase tracking-tighter acct-verified-badge"><?php esc_html_e( 'Verified', 'luxe-landscape' ); ?></span>
						</div>
						<?php endif; ?>
					</div>
				</div>
